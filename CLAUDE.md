...
> 本文件是 Claude Code 每次会话读取的持久记忆。记录「每天都要出去玩」旅游行程规划应用的现状、架构与约定。
> **线上地址**：https://happyforfun.vercel.app ｜ 接 **DeepSeek** 实时生成，端到端跑通 ✅

---

## 1. 这是什么

「每天都要出去玩」——旅游行程规划 Web 应用。核心理念：**输入越少越好、输出越准越好**。
- 想好了去哪 → 直接排详细行程；还没想好 → 帮你选地方、对比、再出行程。
- **数据覆盖**：19 个国家（亚洲 3 + 欧洲 16），多城市；manifest 驱动动态加载（`data/manifest.json` + `data/<id>.json`，新增国家无需改代码）。
- **城市停留规则**：一城 2-3 天、上限 3 天 2 晚（除非用户在 freeText 要求久留）；多城市行程串联城市并标城市间交通。
- **导出**：详细行程可一键导出 Excel（CSV+BOM，零依赖）。
- **多国旅行**：输入端 `multi` 粒度（区域+国家数+必含国）；DEMO 打分引擎(`data/geo.json` 邻接表/常见组合/花费)排陪同国→自动出国家组合→生成跨国行程(顺序优化/总城市数随天数封顶/跨国交通/签证提示)。
- **输出信息流**：逆时间结果流——最新块置顶、点生成回顶等待、块化可折叠、按版本各一条流。
...
- 数据已扩到 19 国（亚洲 3 + 欧洲 16）。**Phase 2 优先批次 8 国已扩到 ≥3 城 + 真实城市间交通(`intercityTransport`)**：日本(东京/京都/大阪+奈良日游点)·意大利(罗马/佛罗伦萨/威尼斯)·法国(巴黎/尼斯/里昂，凡尔赛改 `dayTrip`)·西班牙(巴塞罗那/马德里/塞维利亚)·希腊(雅典/圣托里尼/米科诺斯，含轮渡)·丹麦(哥本哈根/奥胡斯/欧登塞)·瑞典(斯德哥尔摩/哥德堡/马尔默)·挪威(奥斯陆/卑尔根/弗洛姆，含峡湾游船)。其余 ~11 国仍各 2 城，待续扩。部分 suggestedDurationMin/idealDuration 为"按惯例估算"需复核。
- 引擎已支持(Phase 1+2)：`intercityTransport` 真实城市间交通(缺失回落通用建议)；`dayTrip`/`parentCity` 日游点标签(不单独成城)；单国超长→机动/深度日(不截断)；多国最少天数门槛(每国约3天，不足则减国)。
- 限流为进程内基线（多实例需 KV）。
- 节假日数据 2026 单年，需每年更新。覆盖 20 个地区（中国 + 全部 19 目的地国），每条高影响节庆带 `destinationImpact{highlight,note}` 与 source；浮动节日(复活节/希腊东正教复活节/北欧仲夏节)已按 2026 核实。
