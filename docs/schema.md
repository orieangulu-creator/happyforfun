# 行程数据结构说明（Schema · v2.1）

> 文档版本：v2.1（双向匹配版 + 兜底健壮性）
> 配套机器可读版本：`docs/schema.json`（JSON Schema Draft 2020-12，本文与之严格一致）
> 本文用带注释示例 + 字段速查解释全部数据结构，供运营 / AI生成 / UI / 测试 各 agent 对齐。
>
> **v2.1 兜底补洞 Schema 变更**：`UserInput.freeText` 增 `maxLength: 500`（G10）；`DestinationRecommendation.isFallback` 语义修订为"仅全空盲兜底为 true"（G2）；候选新增 `matchLevel`(strong/related)（G3）；顶层新增 `coverageNote`（G3 零匹配提示）；候选 `transport` 补"无 origin 占位"说明（G1）；`TripResult` 新增 `timingWarning`（G4）。设计主线不变。
...
```jsonc
{
  ...
  // [v2.1] 长度上限 500 字(G10); 组装 prompt 时须做边界隔离, 标注为用户输入、不可改系统指令。
...
> - 目的地非 country 但**有任一有效信号**（freeText/标签/时间）→ 定向 `DestinationRecommendation`（`isFallback=false`）。
> - **真正什么都没有**（freeText 空、无标签、无时间、无目的地）→ `DestinationRecommendation` 且 `isFallback=true`（当前季节适合放松的热门地；当前季节由前端按浏览器日期推导，默认北半球/中国，G1/G2）。
...
| `freeText` | string | 否 | 一句话描述，**优先级最高**；**≤500 字**（G10） |
...
```jsonc
{
  ...
  "attractions": [
    {
      ...
      "dayTrip": false,                    // (可选) true=该点是某主城的日游点，不单独成城
      "parentCity": null,                  // (可选) dayTrip=true 时指明所属主城，如 "巴黎"
  ...
  "intercityTransport": [              // (可选) 真实城市间交通，缺失则引擎回落通用建议
    { "from": "东京", "to": "京都", "mode": "train", "durationText": "约2小时15分",
...
| `attractions[]` | array | 是 | 景点，每条带 `source`；可选 `moodTags`、`dayTrip`/`parentCity`(日游点) |
...
目的地未定/模糊时输出 3–5 个候选。**`isFallback` 语义（v2.1 修订 · G2）**：仅当**真正什么都没给**（freeText 空、无标签、无时间、无目的地）才 `isFallback=true`（盲兜底推荐当前季节适合放松的热门地，当前季节按浏览器日期推导）；**只要有任一有效信号**（freeText/标签/时间等），即使结构化目的地为空也是**定向推荐**，`isFallback=false`。

**最少候选策略（v2.1 · G3）**：每个候选标 `matchLevel`（`strong`/`related`）。强匹配不足 3 个时补 `related` 凑 3–5 个；某玩法/区域 **0 个匹配**时**不返回空 `candidates`**，给 v1 已覆盖目的地作候选，并在顶层 `coverageNote` 写明"v1 暂未覆盖该区域/玩法"。

### 4.1 带注释示例

```jsonc
{
  // [v2.1 G2] 仅"真正全空"盲兜底才 true; 有任一信号(freeText/标签/时间)即定向推荐 false
  "isFallback": false,
  "basis": "基于你选的国庆7天假期 + 想放松/想吃标签",
  // [v2.1 G3] 0 匹配/超出 v1 覆盖时给提示, 不返回空结果; 正常匹配可省略或 null
  "coverageNote": null,
  "candidates": [                     // 3-5 个
    {
      "id": "japan",
      ...
      "matchLevel": "strong",         // [v2.1 G3] strong=强匹配 | related=相关推荐(降级补充)
      "matchReason": "10月秋高气爽, 红叶初现; 美食丰富契合“想吃”。",
      "bestVisitTime": "10月下旬-11月(红叶季)",
      "suggestedDays": 6,
      "costTier": "high",             // low | medium | high
      // [v2.1 G1] 无 origin 时 transport=null, UI 交通栏显示占位"填写出发地后估算", 不阻断推荐
      "transport": {
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `isFallback` | boolean | 是 | **仅真正全空盲兜底为 true**；有任一信号即 false（定向推荐）（G2） |
| `basis` | string | 否 | 推荐依据摘要 |
| `coverageNote` | string\|null | 否 (v2.1新增) | 候选不足/0 匹配时的覆盖提示，如"v1 暂未覆盖该区域/玩法"（G3） |
| `candidates[]` | array(1-5) | 是 | 候选列表（即使 0 匹配也不空，至少给 v1 覆盖目的地） |
| `candidates[].matchLevel` | enum | 是 (v2.1新增) | `strong`强匹配 / `related`相关推荐（降级补充）（G3） |
...
| `candidates[].transport` | object\|null | 否 | 从出发地交通；**无 `origin` 时为 null，UI 显示占位文案不阻断**（G1） |
...
> **选择边界（v2.1 · G9）**：本结构 `items` 限 **2–3 个**。选 **1 个**时**不生成对比**，直接进该目的地的 `BestTimeOverview`/`TripResult`；选 **>3 个**时前端拦截并提示"最多对比 3 个"，不生成 >3 项对比。
...
```jsonc
{
  ...
  // [v2.1 新增 G4] 时机警示: 所选时间不在该目的地最佳季节(bestSeasons)时给善意提醒, 行程照常生成。无警示则省略或 null。
  "timingWarning": null,             // 例: "你选的10月在泰国仍属雨季尾声, 最佳季节为11-2月。"
...
| `timingWarning` | string\|null | 否 (v2.1新增) | 踩了非最佳季节时的时机警示（G4），无则省略 |
...
- **`freeText` ≤ 500 字（G10）**；prompt 组装时对其做边界隔离（标注用户输入、不可改系统指令）。
- **`isFallback` 仅全空盲兜底为 true（G2）**；有任一信号即定向推荐 false。盲兜底"当前季节"由前端按浏览器日期推导（默认北半球/中国 · G1）。
- **候选 `matchLevel`(strong/related) + 顶层 `coverageNote`（G3）**：候选不足补 related，0 匹配给 coverageNote，**`candidates` 永不为空**。
- **候选 `transport=null` 表示无 origin（G1）**：UI 显示占位文案，不阻断推荐。
- **`TripResult.timingWarning`（G4）**：所选时间不在 `bestSeasons` 内时给提醒，行程照常。
- **节假日数据按 `(region, year)` 逐份维护，缺年就不提供（G7）**：前端遇缺年优雅降级（隐藏高亮+提示），不造假数据占位。
- **AI 输出须可被 schema.json 校验（G5）**：前端校验失败重试 1 次再报错；断网时静态 `HolidayCalendar` 高亮不受影响。
- **输入校验（G6）**：`end≥start`、过去日期提示、`days` 远超 `idealDuration.maxDays` 时给"建议 X 天"并借 `flexibility` 裁剪。
